...
def check_irreducible_dissection(dissection):
    """To be used in the grammar.

    :param dissection:
    :return: True if the dissection is irreducible, False otherwise.
    """
    # TODO: call the method from the class
    return True


class IrreducibilityChecker:

    def check_irreducible_dissection(self, init_half_edge):
        ...
        # TODO check which half edge is returned by the closure in order to know how to get the opposite outer edge!!
        outer_vertex_half_edge = HalfEdge() # FIX

        # Creates the queue for the BFS.
        queue = list()
        # Put the init half edge into the queue.
        queue.append((init_half_edge, 0, init_half_edge.is_hexagonal is False, set()))
        ...
        while len(queue) != 0:
            ...
            has_been_inner_edge_included = top_element[3]
            visited_nodes = top_element[4]
